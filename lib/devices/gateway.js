...
const Gateway = Thing.type(Parent => class Gateway extends Parent.with(MiioApi, Children) {
	...
	static availableAPI(builder) {
		...
		builder.action('playMusic')
			.done();

		builder.action('stopMusic')
			.done();

		builder.action('deleteMusic')
			.done();

		builder.action('downloadMusic')
			.done();

		builder.action('getMusicList')
			.done();
	...
	getMusicList() {
		return Promise.all([
			this.call('get_music_info', [0]),
			this.call('get_music_info', [1]),
			this.call('get_music_info', [2]),
			this.call('get_music_info', [3])
		])
			.then(results => results.map(result => result.list))
			.then(results => [].concat.apply([], results))
			.then(results => [...new Set(results.map(item => item.index))]);
	}

	deleteMusic(id) {
		return this.call('delete_user_music', [id.toString()])
			.then(MiioApi.checkOk);
	}

	downloadMusic(id, url) {
		return this.call('download_user_music', [id.toString(), url])
			.then(MiioApi.checkOk)
			.then(() => this.waitDownloadMusic(id.toString()));
	}

	downloadProgress() {
		return this.call('get_download_progress', []);
	}

	waitDownloadMusic(id) {
		return this.downloadProgress()
			.then(progress =>
				progress
					...
					.reduce((res, item) => res[item.id] = parseInt(item.progress, 10), {})
			)
			.then(progress => progress[id])
			.then(progress => {
				if (progress === undefined || progress !== 100) {
					return this.wait(50).then(this.waitDownloadMusic(id));
				}
			}).then(() => undefined);
	}
